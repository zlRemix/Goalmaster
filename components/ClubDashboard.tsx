    ...
    const getPaths = () => {
        switch (type) {
            case InfrastructureType.STADIUM:
                return (
                    <g>
                        <ellipse cx="75" cy="85" rx="55" ry="35" fill="none" stroke="currentColor" strokeWidth="4" />
                        <ellipse cx="75" cy="85" rx="35" ry="20" fill="none" stroke="white" strokeWidth="2" opacity="0.3" />
                        <path d="M40 60 L110 60 M40 110 L110 110 M75 50 V120" stroke="white" strokeWidth="1" opacity="0.2" />
                    </g>
                );
            case InfrastructureType.TRAINING_GROUND:
                return (
                    <g>
                        <rect x="35" y="80" width="80" height="15" rx="2" fill="currentColor" />
                        <circle cx="45" cy="87.5" r="15" fill="none" stroke="white" strokeWidth="6" opacity="0.4" />
                        <circle cx="105" cy="87.5" r="15" fill="none" stroke="white" strokeWidth="6" opacity="0.4" />
                        <path d="M75 50 L85 65 H65 Z" fill="white" />
                    </g>
                );
            case InfrastructureType.MEDICAL_CENTER:
                return (
                    <g>
                        <rect x="55" y="55" width="40" height="60" rx="4" fill="none" stroke="currentColor" strokeWidth="4" />
                        <path d="M65 85 H85 M75 75 V95" stroke="white" strokeWidth="6" strokeLinecap="round" />
                        <path d="M40 85 H55 M95 85 H110" stroke="white" strokeWidth="2" opacity="0.3" />
                    </g>
                );
            case InfrastructureType.MARKETING_DEPARTMENT:
                return (
                    <g>
                        <path d="M40 100 L60 60 L90 60 L110 100 Z" fill="none" stroke="currentColor" strokeWidth="4" />
                        <circle cx="75" cy="75" r="10" fill="white" opacity="0.5" />
                        <path d="M50 110 H100" stroke="white" strokeWidth="4" strokeLinecap="round" />
                        <path d="M75 45 V60" stroke="currentColor" strokeWidth="2" />
                    </g>
                );
            default:
        ...
        <div className="space-y-6 md:space-y-8 animate-in fade-in duration-300">
            <header className="bg-slate-900 border-2 border-slate-800 p-6 rounded-[3rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                    ...
                    <div className="text-center md:text-left">
                        ...
                        <span className="bg-blue-500/10 text-blue-400 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-blue-500/20">Profi-Club</span>
                    </div>
                </div>
                <div className="bg-slate-950 px-8 py-4 rounded-[2rem] border border-white/5 text-center shadow-inner">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-1 flex items-center justify-center gap-2"><Euro className="w-3 h-3" /> Vereinsbudget</p>
                    <p className="text-3xl font-black text-white italic tabular-nums leading-none">{(club?.budget || 0).toLocaleString('de-DE')} €</p>
                </div>
            </header>
            ...
            <div className="flex flex-wrap gap-2 p-1.5 bg-slate-950 rounded-3xl border border-slate-800">
                {(['infrastructure', 'squad', 'training', 'management', 'logo'] as ClubNavView[]).map((nav) => {
                    ...
                    return (
                        <button key={nav} onClick={() => setClubNav(nav)} className={`flex-1 min-w-[120px] font-black uppercase text-[10px] tracking-widest py-3 rounded-2xl transition-all ${clubNav === nav ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>
                            {labels[nav]}
                        </button>
                    );
                })}
