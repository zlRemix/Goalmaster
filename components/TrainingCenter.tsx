  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-300">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 className="text-2xl md:text-3xl font-black text-white">Trainingsgelände</h2>
                <p className="text-slate-400 text-sm md:text-base">Investiere TP, um deine Skills zu verbessern.</p>
            </div>
            <div className="bg-gradient-to-tr from-blue-500 to-indigo-500 px-4 py-2 rounded-xl font-bold shadow-lg flex items-center gap-2 self-start md:self-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            {relevantSkills.length > 0 ? relevantSkills.map((skill) => {
                ...
                return (
                    <div key={skill} className="bg-slate-800/80 rounded-2xl p-4 border border-slate-700 flex items-center justify-between group transition-all shadow-md hover:border-blue-500/50">
                        <div className="space-y-2 flex-1 mr-3">
                            <div className="flex justify-between items-center mb-1">
                                ...
                                <div className="text-right">
                                    ...
                                    <span className="text-blue-400 font-mono font-black text-xl">{value}</span>
                                </div>
                            </div>
                           ...
                           <p className="text-xs text-slate-500 font-medium">{pointsToNext > 0 ? `${pointsToNext} bis Rang ${tier + 1}` : 'Maximaler Rang erreicht'}</p>
                        </div>
                        <button onClick={() => onTrain(skill)} disabled={!canAfford} className={`h-16 w-16 rounded-lg flex flex-col items-center justify-center font-bold transition-all z-10 flex-shrink-0 ${
                            canAfford ? 'bg-blue-600 hover:bg-blue-500 text-white cursor-pointer active:scale-90 border border-blue-500/50' : 'bg-slate-800 text-slate-600 cursor-not-allowed opacity-60 border border-slate-700'}`}>
                            <Plus className="h-6 w-6" />
                            <span className="text-xs font-bold uppercase">{cost} TP</span>
                        </button>
                    </div>
                );
            }) : (
                <div className="lg:col-span-2 bg-slate-800/80 rounded-2xl p-8 border border-blue-500/30 text-center">
                    <h3 className="text-lg font-bold text-blue-400">Keine Skills für Position</h3>
