...
const LogoEditor: React.FC<LogoEditorProps> = ({ club }) => {
  ...
  return (
    <div className="bg-slate-800/80 p-6 rounded-3xl border border-slate-700">
      <h3 className="text-lg font-bold text-white mb-4">Logo-Editor</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
            ...
            <div className="flex gap-2 flex-wrap">
              {(['shield', 'circle', 'square', 'hexagon', 'heptagon'] as const).map(shape => (
                <button 
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold transition-colors ${logo.shape === shape ? 'bg-emerald-500 text-white' : 'bg-slate-700 hover:bg-slate-600 text-slate-200'}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
                <label htmlFor="primaryColor" className="block text-sm font-medium text-slate-300 mb-2">Primärfarbe</label>
                <input type="color" id="primaryColor" value={logo.primaryColor} onChange={e => setLogo({...logo, primaryColor: e.target.value})} className="w-full h-10 rounded-lg p-1 bg-slate-700 border-slate-600" />
            </div>
            <div>
                <label htmlFor="secondaryColor" className="block text-sm font-medium text-slate-300 mb-2">Sekundärfarbe</label>
                <input type="color" id="secondaryColor" value={logo.secondaryColor} onChange={e => setLogo({...logo, secondaryColor: e.target.value})} className="w-full h-10 rounded-lg p-1 bg-slate-700 border-slate-600" />
            </div>
          </div>

          {/* Icon Selection */}
          <div>
             <label htmlFor="iconSearch" className="block text-sm font-medium text-slate-300 mb-2">Icon auswählen</label>
             <input 
                type="text"
                id="iconSearch"
                placeholder="z.B. Football, Crown, Castle..."
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
                className="w-full bg-slate-900 border-slate-700 rounded-lg p-2 text-white"
              />
            <div className="h-48 overflow-y-auto mt-2 p-2 bg-slate-900/50 rounded-lg border border-slate-700/50 grid grid-cols-8 gap-2">
                {filteredIcons.map(iconName => {
                    ...
                    return (
                        <button 
                            ...
                            title={iconName}
                            className={`aspect-square flex items-center justify-center rounded-lg transition-colors ${logo.icon === iconName ? 'bg-emerald-500/30 ring-2 ring-emerald-500' : 'bg-slate-800 hover:bg-slate-700'}`}>
